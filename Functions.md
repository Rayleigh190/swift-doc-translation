...
아래 예시의 함수는 사람의 이름을 입력으로 받아서 그 사람의 인사말을 반환한다. 그래서 `greet(person:)`이라고 한다. 이 함수를 만들기 위해서는 하나의 입력 파라미터(`person`이라는 `String` 값)와 인사말을 포함하는 `String` 타입의 반환 값을 정의해야 한다:
...
`greet(person:)` 함수의 body는 `greeting`이라는 새 String 상수(constant)를 정의하고 간단한 인사말 메시지로 설정하는 것을 시작한다. 그런 다음 이 인사말은 return 키워드를 사용하여 함수 밖으로 전달된다. `return greeting`이라는 코드에서 함수는 실행을 마치고 `greeting`의 현재 값을 반환한다.
...
`greet(person:alreadyGreeted:)` 함수는 String 아규먼트 값인 `person`과 `Bool` 아규먼트 값인 `alreadyGreeted`를 괄호로 묶고 쉼표로 구분하여 전달하고 호출한다. 위 함수는 이전 섹션에서 보여준 `greet(person:)` 함수와 다르다는 것을 유의하자. 두 함수 모두 `greet`라는 이름을 가지고 있지만 `greet(person:alreadyGreeted:)` 함수는 두 개의 아규먼트를 받는 반면 `greet(person:)` 함수는 하나만 받는다.
...
`minMax(array:)` 함수의 body는 `currentMin`과 `currentMax`라는 두 개의 변수를 배열의 첫 번째 값(정수)으로 설정하는 것으로 시작한다. 그런 다음 함수는 배열의 첫 번째를 제외한 나머지 값을 반복하고 각 값이 `currentMin`과 `currentMax` 값보다 작거나 큰지 확인한다. 결과적으로 두 개의 최대 최소 Int 값이  튜플로 반환된다.
...
다음은 `(Int) -> Int`를 반환하는 `chooseStepFunction(backward:)`이라는 함수이다. `chooseStepFunction(backward:)` 함수는 `backward`라는 Boolean 파라미터를 기반으로 `stepForward(_:)` 함수 또는 `stepBackward(_:)` 함수를 반환한다:
...
이제 `chooseStepFunction(backward:)`을 사용하여 한 방향 또는 다른 방향으로 이동하는 함수를 얻을 수 있다:
...
위 예에서는 `currentValue`라는 변수를 점진적으로 0에 가깝게 이동하려고 positive 또는 negative 단계 여부를 판별한다. `currentValue`의 초기값은 3이다. 즉, `currentValue > 0`은 `true`를 반환하고, `chooseStepFunction(backward:)`은 `stepBackward(_:)` 함수를 반환한다. 반환된 함수에 대한 참조는 `moveNearerToZero`라는 상수에 저장된다.

이제 `moveNearerToZero` 함수를 사용하여 0까지 셀 수 있다.
...
## [중첩 함수](https://docs.swift.org/swift-book/documentation/the-swift-programming-language/functions/#Nested-Functions)

이 챕터에서 지금까지 만난 모든 함수는 전역 범위에서 정의된 전역 함수의 예였다. 다른 함수의 본문 내부에 함수를 정의할 수도 있는데, 이를 중첩 함수라 한다.

중첩된 함수는 기본적으로 외부로부터 숨겨져 있지만, 해당 함수를 둘러싼 함수에서는 호출하여 사용할 수 있다. 둘러싼 함수는 중첩된 함수 중 하나를 반환하여 중첩된 함수를 다른 범위에서 사용할 수 있도록 할 수도 있다.

위의 `chooseStepFunction(backward:)` 예제를 다시 작성하여 중첩된 함수를 사용하고 반환할 수 있다:
